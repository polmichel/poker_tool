...
class Deck:
    ...
    def __init__(self, excluded: list[str] | None = None, rng: random.Random | None = None) -> None:
    ...
    def draw(self, count: int) -> list[str]:
        ...
        available = [f"{rank}{suit}" for rank in RANKS for suit in SUITS if f"{rank}{suit}" not in self._excluded]
        if count > len(available):
            raise ValueError(f"Cannot draw {count} cards: only {len(available)} available")
