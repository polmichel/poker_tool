...
class TestHand(unittest.TestCase):
    ...
    def test_generate_all_hands_count(self):
        """generate_all_hands returns all 169 canonical hands (13x13 grid)."""
        all_hands = generate_all_hands()
        # 169 = 13 pairs + 156 (suited + offsuit for each non-pair rank combo).
        self.assertEqual(len(all_hands), 169)
        # All should be Hand instances.
        for hand in all_hands:
            self.assertIsInstance(hand, Hand)

    def test_generate_all_hands_contains_suited_and_offsuit(self):
        """For every non-pair, both suited and offsuit variants are present."""
        all_hands = generate_all_hands()
    ...
    def test_generate_all_hands_unique(self):
