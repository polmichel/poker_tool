...
export class TrainingApi {
  ...
  async modes(): Promise<TrainingModeResponse> {
    try {
      ...
      throw new Error(extractErrorMessage(error, 'Failed to fetch training modes'));
    }
  }
  ...
  async sessions(): Promise<TrainingSession[]> {
    try {
      ...
      return validateApiResponse<TrainingSessionsResponse>(
        TrainingSessionsResponseSchema,
        response.data,
      );
    } catch (error) {
      throw new Error(extractErrorMessage(error, 'Failed to fetch training sessions'));
    }
  }
  ...
  async createSession(payload: CreateSessionRequest): Promise<CreateSessionResponse> {
    try {
      ...
      throw new Error(extractErrorMessage(error, 'Failed to create training session'));
    }
  }
  ...
  async session(sessionId: number): Promise<SessionDetail> {
    try {
      ...
      throw new Error(extractErrorMessage(error, `Failed to fetch session ${sessionId}`));
    }
  }
  ...
  async start(
    ...
    try {
      ...
      throw new Error(extractErrorMessage(error, `Failed to start session ${sessionId}`));
    }
  }
  ...
  async end(sessionId: number): Promise<{ message: string; session: TrainingSession }> {
    try {
      ...
      throw new Error(extractErrorMessage(error, `Failed to end session ${sessionId}`));
    }
  }
  ...
  async sessionsByUser(userId: number): Promise<TrainingSession[]> {
    try {
      ...
      throw new Error(extractErrorMessage(error, `Failed to fetch sessions for user ${userId}`));
    }
  }
}
