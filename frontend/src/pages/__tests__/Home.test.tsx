...
jest.mock('../../components', () => ({
  AppCard: ({ entry, onSelect }: { entry: any; onSelect: (entry: any) => void }) => (
    <button onClick={() => onSelect(entry)} data-testid={`app-card-${entry.slug}`}>
      {entry.title}
    </button>
  ),
...
describe('Home hub', () => {
  ...
  test('renders donation button', () => {
    renderHome();
    expect(screen.getByText('Faire un don au d\u00e9veloppeur')).toBeInTheDocument();
  });

  test('clicking donation button opens donation dialog', () => {
    renderHome();
    fireEvent.click(screen.getByText('Faire un don au d\u00e9veloppeur'));
